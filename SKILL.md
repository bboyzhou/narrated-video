...
- 需要生成式人物动作时，先由已批准分镜声明 `asset_strategy: generated_video`、`source_image`、`motion_prompt`、`motion_constraints` 和 `generation`，再使用 `video-prepare` 生成任务包；外部 provider 完成后用 `video-import` 导入。默认 provider 为 `wan22_kaggle`，使用固定 Kaggle Worker 在一次模型加载中批量处理当前阶段的镜头；旧 `cogvideox_colab` 仍兼容。外部生成不加入本地 `preflight`，不需要本地 CUDA、torch 或模型权重。生成失败时按“已批准生成视频 → 已批准本地/现有视频 → 已批准分镜图片加原有运镜”顺序降级，不阻塞全片。
