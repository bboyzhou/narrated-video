...
- 把生成模型视为“短镜头动态素材生成器”，不是整段视频渲染器；每个生成镜头必须有已批准的图片运镜或本地视频 fallback。
...
- 需要生成式人物动作时，先由已批准分镜声明 `asset_strategy: generated_video`、`source_image`、`motion_prompt`、`motion_constraints` 和 `generation`，再使用 `video-prepare` 生成 v3 任务包；Job 只携带内容、语义运动、seed、`target_duration_sec` 与输出路径。项目级分别选择 `provider`（如 `skyreels_v2/wan22/cogvideox/svd_xt`）、通用 `profile` 和 `runtime.type`。实际执行参数只出现在系统生成的 `RuntimePlan`。旧组合 ID 仅用于读取迁移，不写入新任务包。生成失败统一记录原因并降级到已批准的 Remotion/本地素材 fallback，不阻塞全片。
