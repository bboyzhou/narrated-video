...
1. 先确认当前项目已有成功且未失效的 `preflight`；没有时停止分镜设计并处理运行环境。
2. 从已批准口播稿提炼制作纲要，不新增未经查证的事实。
3. 先按叙事节拍划分镜头，再写画面；一个镜头可以覆盖多句，但所有句子必须按原顺序覆盖一次。
4. 为全部镜头填写 `storyboard.json`，同步项目 `shots` 中的口播映射、正负提示词、运镜与转场。
5. 运行 `check`，处理结构错误和时长警告。口播阶段使用 `check --stage script`；完整分镜阶段使用默认 `check`。
6. 向用户展示制作纲要、完整逐镜表、Demo 选段理由和验证目标，等待实际批准。
7. 批准后记录：
