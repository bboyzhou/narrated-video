...
- 每句 WAV 在拼接前统一归一化到约 -20 LUFS、真峰值 -2 dB，并施加`dynaudnorm` 平滑并补偿句内电平；归一化不应改变自然停顿。可选 `pacing.rate` 对有效发声区间做软语速归一化：按中文口语单位/有效发声秒数估算语速，仅修正超出目标容差的句子，并将 `atempo` 限制在配置范围内；短句和显式 `rate_policy: preserve` 的句子只报告不调整，`rate_policy: normalize` 可让单句覆盖全局关闭策略。推荐配置为 `target_units_per_second: 4.5`、`tolerance: 0.12`、`min_tempo: 0.88`、`max_tempo: 1.12`。可选 `pacing` 为句尾增加模型无关的语义停顿：普通句 `default_pause` 0.28 秒，冒号/分号续接 0.18 秒，对话/强调 0.4 秒，切镜 0.5 秒，结尾 1.5 秒；句子可用 `pause_after` 或 `pause_role` 覆盖。`respect_existing_tail: true` 时只补齐现有尾部静音不足，不重复叠加 MeloTTS 自带停顿。字幕默认在讲话结束时消失，`subtitle_during_pause: true` 才覆盖停顿。以语速归一化、停顿处理后的真实样本数测量时长，再向上对齐整数视频帧，只在句尾补不足一帧的静音，不裁掉讲话。时间轴和 SRT 使用这些帧边界。因此长片总时长可能比原 WAV 时长之和多出少量补齐时间。验证报告会记录每句原始/归一化语速和调速因子。
