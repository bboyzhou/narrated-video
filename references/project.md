...
- 字幕每句一个时间区间，可折成两行，不伪造逐字时间。单句超过 `2 * max_chars` 时需在真实口播停顿处分句，更新批准稿/分句审批后重跑。size、margin 是 720p 参考像素值，随分辨率缩放。
...
- `music` 为多段音轨，可重叠；不足长度自动循环，分别淡入淡出，并依据解说进行 sidechain ducking。start/end 使用全片秒数。解说统一响度至目标 -18 LUFS，混音加峰值限制器；仍需试听，脚本不保证任意素材都具有同等听感。
