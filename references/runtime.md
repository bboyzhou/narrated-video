...
`init` 后即可运行 `paths/configure/doctor`，不需要先填写文案和镜头。`paths` 只展示项目、全局运行环境和当前进程中的候选，不选择、不写配置，也不是全盘资源搜索工具。agent 应结合用户给定安装目录及已有项目进一步查找，集中展示候选后请用户选择。
...
`configure` 记录选择，不代替用户做选择；只验证文件/目录存在，不导入模型或下载资源。`doctor` 通过后，用户明确同意时再运行 `remember-runtime`，将已验证的路径保存到当前用户的全局配置文件 `${CODEX_HOME}/narrated-video/runtime.json`（未设置时为 `~/.codex/narrated-video/runtime.json`）。这个文件供不同 Agent 会话复用，不修改系统 PATH，也不需要管理员权限。
...
- runtime 变化会使 Demo 批准和缓存失效，避免换解释器或资源后误用旧配音。模型在原位置更新时仍需递增 `voice.revision`。`remember-runtime` 只写用户自己的全局配置文件，不修改系统级环境变量；删除或改名该文件即可清除记忆路径。
